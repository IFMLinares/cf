...
class checkOutView(LoginRequiredMixin, CartMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            shipping_address = Address.objects.filter(user=request.user, show=True)
            billing_address = BillingAddress.objects.filter(user=request.user, show=True)
            if shipping_address.exists():
                context['shipping_address'] = shipping_address[0]
                
            if billing_address.exists():
                context['billing_address'] = billing_address[0]
